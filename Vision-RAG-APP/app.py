...
with st.sidebar:
    ...
    groq_api_key = st.text_input("groq API Key (Groq)", type="password", key="groq_key")
...
if cohere_api_key and groq_api_key:
    try:
...
with st.expander("ℹ️ About the models used"):
    ...
    ### Groq GPT-OSS-120B
    GPT-OSS-120B is a powerful 120 billion parameter language model that excels in understanding and generating human-like text. 
    It's designed for high-performance tasks like question-answering, summarization, and content creation.
    Running on Groq's advanced hardware architecture, it delivers fast inference times while maintaining high accuracy, making it ideal for real-time applications like this RAG system.
    """)
...
# Answer function
def answer(question: str, img_path: str, groq_client) -> str:
    """Answers the question based on the retrieved image using Groq (text-only approach)."""
    if not groq_client or not img_path or not os.path.exists(img_path):
        missing = []
        if not groq_client: missing.append("Groq client")
    ...
    try:
        # Since Groq doesn't support vision models directly, we'll provide context about the retrieved image
        img_filename = os.path.basename(img_path)
        
        # Extract context from filename (useful for the sample images)
        context_hints = []
        if "tesla" in img_filename.lower():
            context_hints.append("This appears to be related to Tesla company data/financials")
        elif "netflix" in img_filename.lower():
            context_hints.append("This appears to be related to Netflix company data/financials")
        elif "nike" in img_filename.lower():
            context_hints.append("This appears to be related to Nike company data/financials")
        elif "google" in img_filename.lower():
            context_hints.append("This appears to be related to Google company data/financials")
        elif "accenture" in img_filename.lower():
            context_hints.append("This appears to be related to Accenture company data/financials")
        elif "tecent" in img_filename.lower():
            context_hints.append("This appears to be related to Tencent company data/financials")
        elif "page_" in img_filename.lower():
            context_hints.append("This appears to be a page from a PDF document")

        context_str = " ".join(context_hints) if context_hints else "This is a retrieved image from the document collection"

        prompt = f"""Based on the retrieved image context: {context_str}

Question: {question}

Please provide a helpful answer based on what you can infer from the context. If this relates to financial data or company information, provide relevant general knowledge about the topic. Be as elaborate as possible giving extra relevant information. Don't use markdown formatting in the response. Please provide enough context for your answer.

Note: The actual image content cannot be directly analyzed by this model, but the image was selected as most relevant to your question from the embedded document collection."""
...
run_button = st.button("Run Vision RAG", key="main_run_button", 
                      disabled=not (cohere_api_key and groq_api_key and question and st.session_state.image_paths and st.session_state.doc_embeddings is not None and st.session_state.doc_embeddings.size > 0))
...
if run_button:
    if co and groq_client and st.session_state.doc_embeddings is not None and len(st.session_state.doc_embeddings) > 0:
            ...
            # Ensure embeddings and paths match before search
             if len(st.session_state.image_paths) != st.session_state.doc_embeddings.shape[0]:
                ...
                if top_image_path:
                    ...
                    retrieved_image_placeholder.image(top_image_path, caption=caption, use_column_width=True)

                    with st.spinner("Generating answer..."):
                        final_answer = answer(question, top_image_path, groq_client)
...
# Footer
st.markdown("---")
st.caption("Vision RAG with Cohere Embed-4 | Built with Streamlit, Cohere Embed-4, and Groq")